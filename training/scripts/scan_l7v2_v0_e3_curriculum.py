#!/usr/bin/env python3
"""
Curriculum scan for E3-v0 (marker patch): sweep clue_len (and optionally seq_len).

Motivation (per temp-16):
  - E3-v0 on long sequences can become evidence-sparse (hit_rate ~ geometric chance).
  - Before changing mechanisms, locate the regime where:
      - hit_rate is sufficient
      - probe_auc rises above chance
      - dwell emerges (fixation)

This script sweeps (seed × clue_len) for fixed seq_len and records E3 metrics.

Outputs:
  - results.jsonl (one line per run)
  - grid_summary.json (aggregated per clue_len)
"""
...
def main() -> None:
    ...
    ap.add_argument("--save_root", default="checkpoints/l7v2_v0_e3_curriculum")
    ap.add_argument("--timeout_s", type=int, default=7200)

    ap.add_argument("--seeds", type=str, default="0,1,2")
    ap.add_argument("--clue_lens", type=str, default="160,128,96,64,32")
    ap.add_argument("--seq_len", type=int, default=512)
    ap.add_argument("--clue_margin", type=int, default=32)
    ap.add_argument(
        "--policy",
        type=str,
        default="active",
    ...
    clue_lens = _parse_csv_ints(str(args.clue_lens))
    ...
    total = len(seeds) * len(clue_lens)
    idx = 0
    for seed in seeds:
        for clue_len in clue_lens:
            idx += 1
            trial_root = os.path.join(save_root, f"seed{seed}", f"clue{clue_len}")
            cmd = base + [
                "--save_dir",
                trial_root,
                "--seed",
            ...
            print(f"[scan] ({idx}/{total}) seed={seed} clue_len={clue_len}")
            ...
            fr = FlatResult(seed=int(seed), clue_len=int(clue_len), run_dir=str(run_dir), elapsed_s=float(elapsed), summary=summary)
            flat.append(fr)
            rec["E3"] = fr.e3()
            _write_jsonl(results_path, rec)

    # Aggregate per clue_len
    by_clue: Dict[int, List[FlatResult]] = {}
    for fr in flat:
        by_clue.setdefault(int(fr.clue_len), []).append(fr)
    ...
    grid: Dict[str, Any] = {}
    for clue_len, rs in sorted(by_clue.items(), key=lambda kv: kv[0], reverse=True):
        ...
        grid[str(int(clue_len))] = {k: mean(v) for k, v in vals.items()}
