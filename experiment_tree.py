...
class SoftTreePotential:
    tree: SyntheticTree
    L_target: float = 1.7
    k_att: float = 2.0
    A_rep: float = 5.0     # Re-enabled higher repulsion for unfolding
    sigma_rep: float = 0.5
    eps_soft: float = 0.2  # Hamiltonian Regularization
    ...
    def gradient(self, Z_uhp, action=None):
        # Fully Vectorized Gradient with Regularization
        Z = np.asarray(Z_uhp, dtype=np.complex128).ravel()
        n = Z.size
...
def run_tree_experiment():
    print("Setting up Tree Experiment (Physics Mode - Soft Core)...")
    depth = 3
    tree = SyntheticTree(depth=depth)
    
    pot = SoftTreePotential(tree=tree, L_target=1.7, k_att=2.0, A_rep=5.0, sigma_rep=0.5, eps_soft=0.2)
