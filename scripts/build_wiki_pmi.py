...
def main():
    ...
    # --- PHASE 3: PMI ---
    print(f"[{time.strftime('%X')}] Calculating PMI...", flush=True)
    ...
    coverage = len(active_nodes) / vocab_size * 100.0 if vocab_size > 0 else 0
    print(f"Total Edges: {len(edges_to_save):,}", flush=True)
    print(f"Coverage: {coverage:.2f}%", flush=True)
    
    degrees = []
    for u in active: # Wait, active is just set of nodes. Recover degrees from neighbors?
        # Re-calc degree for P99
        d = 0
        for v, s in nbrs[:TOP_K]: # Wrong scope
             pass
    # Correct P99 logic:
